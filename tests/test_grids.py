...
def test_Grid_get_subgrids():
    ...
    conditions_dicts = {
        ...
        'no_condition': {},
        'greater_than_one': dict(
            (key, greater_than_one) for key in dimensions.keys()
        ),
    }
...
def test_get_as_subsubgrid_equal_grids():
    ...
    for dimension in subsubgrid.dimensions:
        assert torch.equal(
            subsubgrid.dimensions[dimension],
            subgrid.dimensions[dimension]
        ), dimension
        assert not dimension in subsubgrid.indices_dict
...
def test_get_as_subsubgrid_missing_coords():
    ...
    with pytest.raises(ValueError) as e_info:
        subsubgrid = grids.get_as_subsubgrid(subgrid2, subgrid1, copy_all=False)
